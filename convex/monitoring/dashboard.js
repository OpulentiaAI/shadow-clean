...
exports.getRecentTraces = exports.getFeatureFlags = exports.checkAlerts = exports.getSystemHealth = void 0;
const server_1 = require("../_generated/server");
/**
 * Get real-time system health metrics
 */
exports.getSystemHealth = (0, server_1.query)({
    args: {},
    handler: async (ctx) => {
        const now = Date.now();
 ...
 */
exports.checkAlerts = (0, server_1.query)({
    args: {},
    handler: async (ctx) => {
        const now = Date.now();
        ...
        const stuckWorkflows = await ctx.db
            .query("workflowTraces")
            .withIndex("by_status")
            .filter((q) => q.and(q.eq(q.field("status"), "STARTED"), q.lt(q.field("startedAt"), fiveMinutesAgo)))
            .collect();
        ...
        const stuckMessages = await ctx.db
            .query("chatMessages")
            .withIndex("by_status")
            .filter((q) => q.and(q.eq(q.field("status"), "streaming"), q.lt(q.field("createdAt"), fiveMinutesAgo)))
            .collect();
 ...
 */
exports.getFeatureFlags = (0, server_1.query)({
    args: {},
    handler: async () => {
        return {
 ...
 */
exports.getRecentTraces = (0, server_1.query)({
    args: {},
    handler: async (ctx) => {
        const traces = await ctx.db
            .query("workflowTraces")
