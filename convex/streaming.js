...
exports.resumeStream = exports.stopTask = exports.cancelStream = exports.streamChatWithTools = exports.streamChat = void 0;
// @ts-nocheck
 ...
 */
async function fetchConversationContext(ctx, taskId, excludeMessageId, excludePromptMessageId) {
    const messages = await ctx.runQuery(api_1.api.messages.byTask, { taskId });
    ...
    const completedMessages = messages.filter((m) => {
        ...
        if (excludeMessageId && m._id === excludeMessageId)
            return false;
        // Exclude the prompt message we just saved (will be added separately to LLM)
        if (excludePromptMessageId && m._id === excludePromptMessageId)
            return false;
...
function resolveProvider({ model, apiKeys }) {
    ...
    console.log(`[STREAMING]   - nvidia: ${apiKeys.nvidia ? `YES (${apiKeys.nvidia.length} chars)` : "NO"}`);
    console.log(`[STREAMING]   - fireworks: ${apiKeys.fireworks ? `YES (${apiKeys.fireworks.length} chars)` : "NO"}`);
    ...
    if (model.startsWith("gateway:")) {
        ...
        const gatewayClient = (0, openai_1.createOpenAI)({
            apiKey: gatewayKey,
    ...
    if (model.startsWith("accounts/fireworks/")) {
        ...
        const fireworksClient = (0, openai_1.createOpenAI)({
            apiKey: fireworksKey,
    ...
    if (model.startsWith("nim:")) {
        ...
        const nimClient = (0, openai_1.createOpenAI)({
            apiKey: nvidiaKey,
    ...
    if (apiKeys.anthropic) {
        ...
        return (0, anthropic_1.createAnthropic)({ apiKey: apiKeys.anthropic })(model);
    }
    if (apiKeys.openai) {
        console.log(`[STREAMING] Using OpenAI with client-provided API key`);
        return (0, openai_1.createOpenAI)({ apiKey: apiKeys.openai })(model);
    }
    ...
    if (envAnthropic) {
        ...
        return (0, anthropic_1.createAnthropic)({ apiKey: envAnthropic })(model);
    }
    const envOpenAI = process.env.OPENAI_API_KEY;
    if (envOpenAI) {
        console.log(`[STREAMING] Using OpenAI with environment API key`);
        return (0, openai_1.createOpenAI)({ apiKey: envOpenAI })(model);
    }
...
function normalizeUsage(usage) {
    if (!usage)
        return undefined;
    const promptTokens = usage.promptTokens ?? usage.inputTokens ?? usage.prompt ?? 0;
    const completionTokens = usage.completionTokens ?? usage.outputTokens ?? usage.completion ?? 0;
 ...
 */
exports.streamChat = (0, server_1.action)({
    args: {
        ...
        apiKeys: values_1.v.object({
            ...
            nvidia: values_1.v.optional(values_1.v.string()),
            fireworks: values_1.v.optional(values_1.v.string()),
        }),
    },
    handler: async (ctx, args) => {
        ...
        const { messageId } = await ctx.runMutation(api_1.api.messages.startStreaming, {
            taskId: args.taskId,
        ...
        try {
            ...
            for await (const delta of result.textStream) {
                ...
                await ctx.runMutation(api_1.api.messages.appendStreamDelta, {
                    messageId,
                    deltaText: delta,
                    isFinal: false,
                });
            ...
            await ctx.runMutation(api_1.api.messages.update, {
                messageId,
            ...
            if (userMessage.includes("No output generated") ||
 ...
 */
exports.streamChatWithTools = (0, server_1.action)({
    args: {
        taskId: values_1.v.id("tasks"),
        prompt: values_1.v.string(),
        model: values_1.v.string(),
        ...
        tools: values_1.v.optional(values_1.v.array(values_1.v.object({
    ...
    handler: async (ctx, args) => {
        ...
        console.log(`[STREAMING] API keys present - anthropic: ${!!args.apiKeys?.anthropic}, openai: ${!!args.apiKeys?.openai}, openrouter: ${!!args.apiKeys?.openrouter}, nvidia: ${!!args.apiKeys?.nvidia}, fireworks: ${!!args.apiKeys?.fireworks}, exa: ${!!args.apiKeys?.exa}`);
        ...
        console.log(`[STREAMING] Task workspacePath: ${task?.workspacePath ?? "null"}`);
        ...
        if (ENABLE_PROMPT_MESSAGE_ID) {
            ...
            const assistantResult = await ctx.runMutation(api_1.api.messages.getOrCreateAssistantForPrompt, {
                taskId: args.taskId,
                promptMessageId,
                llmModel: args.llmModel,
            });
            messageId = assistantResult.messageId;
            console.log(`[STREAMING] ${assistantResult.reused ? "Reused" : "Created"} assistant message: ${messageId} (linked to prompt)`);
            ...
            if (assistantResult.reused) {
                ...
                const shouldReturnExisting = existingStatus === "pending" ||
                    existingStatus === "streaming" ||
                    existingStatus === "complete";
                if (shouldReturnExisting) {
                    const existingTools = await ctx.runQuery(api_1.api.toolCallTracking.byMessage, {
                        messageId,
                    });
                ...
                const retryAssistant = await ctx.runMutation(api_1.api.messages.createAssistantMessage, {
                    taskId: args.taskId,
                    promptMessageId,
                    llmModel: args.llmModel,
                });
                messageId = retryAssistant.messageId;
                console.log(`[STREAMING] Previous assistant message failed; created new assistant message for retry: ${messageId}`);
            }
            // Update status to streaming
            await ctx.runMutation(api_1.api.messages.updateMessageStatus, {
                messageId,
                status: "streaming",
            });
        }
            ...
            const result = await ctx.runMutation(api_1.api.messages.startStreaming, {
                taskId: args.taskId,
                llmModel: args.llmModel,
            });
        ...
        const traceId = (0, observability_1.generateTraceId)();
        ...
        let dbWriteCount = 0;
        // Start trace
        await ctx.runMutation(api_1.api.observability.startTrace, {
            taskId: args.taskId,
            traceId,
        ...
        try {
            ...
            const exaTools = (0, agentTools_1.createExaWebSearchTool)(args.apiKeys?.exa);
            // Fetch enabled MCP connectors and their tools for this user
            let mcpTools = {};
            if (task?.userId) {
                try {
                    const enabledConnectors = await ctx.runQuery(api_1.api.mcpConnectors.listEnabledByUser, {
                        userId: task.userId,
                    });
                    if (enabledConnectors.length > 0) {
                        // Discover tools for each enabled connector
                        const connectorsWithTools = await Promise.all(enabledConnectors.map(async (connector) => {
                            try {
                                ...
                                if (connector.configJson) {
                                    try {
                                        const parsed = JSON.parse(connector.configJson);
                        ...
                        if (validConnectors.length > 0) {
                            mcpTools = (0, agentTools_1.createMcpProxyTools)(validConnectors);
            ...
            const filteredTools = Object.fromEntries(Object.entries(allTools).filter(([name]) => requestedNames ? requestedNames.includes(name) : true));
            if (requestedNames && Object.keys(filteredTools).length === 0) {
                throw new Error(`No matching tools were found for requested tools: ${requestedNames.join(", ")}`);
            }
            // ============================================================
            ...
            const toolCallAttempts = new Map(); // Track attempts per signature
            ...
            const createToolSignature = (toolName, toolArgs) => {
                // Normalize args for comparison (exclude 'explanation' field)
                const argsForSignature = { ...toolArgs };
            ...
            const wrapToolWithDedup = (toolName, originalTool) => {
            ...
            const aiTools = Object.fromEntries(Object.entries(filteredTools).map(([name, tool]) => [
                name,
            ...
            const completedToolSignatures = new Set();
            ...
            const makeToolSignature = (toolName, toolArgs) => {
            ...
            const formatForPrompt = (value) => {
                try {
                    const text = JSON.stringify(value);
                    if (text.length <= MAX_SINGLE_TOOL_RESULT_CHARS)
                        return text;
            ...
            const alreadyReadFiles = new Set();
            const alreadyListedDirs = new Set();
            ...
            const buildContinuationPrompt = () => {
                ...
                if (planSteps.length > 0) {
                    ...
                    const nextIdx = planSteps.findIndex((_, idx) => !completedPlanSteps.has(idx));
...
Review the above results. If you have enough information, provide your response. If not, call ONE different tool with correct arguments.`;
            ...
            const parseToolPlan = (prompt, allowedToolNames) => {
            ...
            const matchesPlanStep = (step, toolName, toolArgs) => {
                if (step.toolName !== toolName)
                    return false;
                for (const [k, v] of Object.entries(step.args)) {
                    const candidate = toolArgs[k];
                    if (stableStringify(candidate) !== stableStringify(v))
                        return false;
                }
                return true;
            };
            const planSteps = parseToolPlan(basePrompt, new Set(Object.keys(aiTools)));
            const completedPlanSteps = new Set();
            const extractJsonArgsFromPrompt = (prompt) => {
            ...
            const ensureToolTracking = async (toolCallId, toolName, toolArgs) => {
                if (knownToolCalls.has(toolCallId))
                    return;
                await ctx.runMutation(api_1.api.toolCallTracking.create, {
                    messageId,
                    taskId: args.taskId,
                    toolName,
                    args: toolArgs,
            ...
            for (let round = 0; round < MAX_AGENT_STEPS; round++) {
                ...
                const promptForRound = round === 0 ? basePrompt : buildContinuationPrompt();
                const roundToolCallIds = new Set();
                ...
                try {
                    ...
                    if (ENABLE_RETRY_WITH_BACKOFF) {
                        result = await (0, retry_1.withRetry)(llmCall, {
                            maxAttempts: 3,
                            initialBackoffMs: 200,
                            base: 2,
                            jitter: true,
                            onRetry: (attempt, error, delay) => {
                                console.log(`[STREAMING] LLM retry ${attempt}/3, waiting ${delay}ms: ${error.message.substring(0, 100)}`);
                            },
                        });
                    }
                    else {
                    ...
                    console.log(`[STREAMING] streamText promise resolved, processing stream... (round=${round}, promptLen=${effectivePrompt.length})`);
                }
                catch (streamTextError) {
                    // Check if it's a transient error for better error messaging
                    const isTransient = (0, retry_1.isTransientError)(streamTextError);
                    console.error(`[STREAMING] streamText failed (round=${round}, transient=${isTransient}):`, streamTextError);
                ...
                for await (const part of result.fullStream) {
                    ...
                    const currentTask = await ctx.runQuery(api_1.api.tasks.get, { taskId: args.taskId });
                    ...
                    roundPartCount++;
                    if (roundPartCount === 1) {
                        console.log(`[STREAMING] Received first stream part, type: ${partType}`);
                    }
                    // Log ALL part types for debugging NVIDIA NIM reasoning
                    if (roundPartCount <= 20 || partType !== "text-delta") {
                        console.log(`[STREAMING] Part #${roundPartCount} type=${partType} keys=${Object.keys(part).join(",")} fullPart=${JSON.stringify(part).substring(0, 300)}`);
                    }
                    ...
                    if (partType === "tool-input-start" ||
                        ...
                        console.log(`[STREAMING] STREAM PART type=${partType} payload=${JSON.stringify(part).substring(0, 500)}`);
                    }
                    ...
                    if (partType === "reasoning-delta" || partType === "thinking-delta") {
                        ...
                        const reasoningDelta = part.delta ?? part.text ?? part.thinking ?? part.reasoning_content ?? "";
                        if (reasoningDelta) {
                        ...
                        const reasoningText = part.reasoning ?? part.thinking ?? part.reasoning_content ?? part.text ?? "";
                        ...
                        const reasoningContent = part.reasoning_content;
                    ...
                    else if (partType === "tool-call-delta" ||
                        ...
                        partType === "tool-call-start") {
                        const rawToolCallId = part.toolCallId ?? part.id;
                        ...
                        const argsText = part.argsText ??
                            part.delta ?? // tool-input-delta
                            part.text ??
                            (typeof part.args === "string"
                                ? part.args
                                : "");
                        const argsObject = part.args ?? part.input ?? undefined;
                        if (!toolCallId)
                            continue;
                        ...
                        await ensureToolTracking(toolCallId, currentState.toolName, currentState.latestArgs);
                        await ctx.runMutation(api_1.api.messages.appendStreamDelta, {
                            messageId,
                            deltaText: "",
                            isFinal: false,
                        ...
                        const toolCallId = toolCallIdRaw
                            ? normalizeToolCallId(toolCallIdRaw)
                            : "";
                        const toolName = part.toolName ?? part.name ?? "unknown-tool";
                        ...
                        if (!toolCallId)
                            continue;
                        roundToolCallIds.add(toolCallId);
                        await ctx.runMutation(api_1.api.toolCallTracking.updateResult, {
                            toolCallId,
                            result: toolResult,
                            status: "COMPLETED",
                        });
                        ...
                        completedToolSignatures.add(makeToolSignature(toolName, toolCallStates.get(toolCallId)?.latestArgs ?? {}));
                        if (planSteps.length > 0) {
                            const nextIdx = planSteps.findIndex((_, idx) => !completedPlanSteps.has(idx));
                ...
                console.log(`[STREAMING] Stream iteration complete, processed ${roundPartCount} parts (round=${round})`);
                ...
                console.log(`[STREAMING] [v6] Checking pending tools (round=${round}): toolStates=${toolCallStates.size}, roundTools=${roundToolCallIds.size}, toolCount=${Object.keys(aiTools).length}`);
                if (roundToolCallIds.size > 0 && Object.keys(aiTools).length > 0) {
                    for (const toolCallId of roundToolCallIds) {
                        ...
                        if (!state)
                            continue;
                        console.log(`[STREAMING] [v6] Tool state: id=${toolCallId}, name=${state.toolName}, argKeys=${Object.keys(state.latestArgs).join(",")}`);
                        const toolDef = aiTools[state.toolName];
                        const execArgs = {
                            ...(state.latestArgs ?? {}),
                        };
                        ...
                        if (Object.keys(execArgs).length === 0) {
                            ...
                            if (recovered) {
                                console.log(`[STREAMING] [v6] Recovered tool args from prompt for ${state.toolName}: ${Object.keys(recovered).join(",")}`);
                                ...
                                if (state.toolName === "list_dir" &&
                                    typeof recovered.relative_workspace_path === "string" &&
                                    typeof recovered.explanation === "string") {
                                    Object.assign(execArgs, recovered);
                                }
                                else if (state.toolName === "read_file" &&
                                    typeof recovered.target_file === "string" &&
                                    typeof recovered.explanation === "string") {
                                    Object.assign(execArgs, recovered);
                                }
                                else if (state.toolName === "grep_search" &&
                                    typeof recovered.query === "string" &&
                                    typeof recovered.explanation === "string") {
                                    Object.assign(execArgs, recovered);
                                }
                                else if (state.toolName === "file_search" &&
                                    typeof recovered.pattern === "string" &&
                                    typeof recovered.explanation === "string") {
                                    Object.assign(execArgs, recovered);
                                }
                            }
                        ...
                        if (state.toolName === "read_file" &&
                            ...
                            for (const pattern of filePatterns) {
                                ...
                                if (match?.[1]) {
                                    ...
                                    console.log(`[STREAMING] [v8] Auto-filled read_file args from prompt: ${match[1]}`);
                                    break;
                                }
                            }
                        }
                        if (toolDef?.execute && Object.keys(execArgs).length > 0) {
                            ...
                            try {
                                ...
                                const candidateTaskWorkspace = task?.workspacePath;
                                const workspacePathOverride = candidateTaskWorkspace || "/workspace";
                                const serverUrl = process.env.SHADOW_SERVER_URL || "http://localhost:4000";
                                const toolApiKey = process.env.CONVEX_TOOL_API_KEY || "shadow-internal-tool-key";
                                let toolResult;
                                ...
                                const callToolApiWithWorkspaceFallback = async () => {
                                    console.log(`[STREAMING] [v7] Using direct tool API call (tool=${state.toolName}, workspaceOverride=${workspacePathOverride})`);
                                    const callOnce = async (workspacePath) => {
                                        const resp = await fetch(`${serverUrl}/api/tools/${args.taskId}/${state.toolName}`, {
                                            method: "POST",
                                            headers: {
                                    ...
                                    if (first?.success === false &&
                                        ...
                                        first.error.includes("ENOENT") &&
                                        workspacePathOverride !== "/workspace") {
                                        console.log(`[STREAMING] [v7] ${state.toolName} ENOENT on ${workspacePathOverride}, retrying with /workspace`);
                                        return await callOnce("/workspace");
                                    }
                                    return first;
                                };
                                const toolSignature = makeToolSignature(state.toolName, execArgs);
                                if (completedToolSignatures.has(toolSignature)) {
                                    ...
                                    const isScratchpad = task?.isScratchpad === true;
                                ...
                                console.log(`[STREAMING] [v6] Tool result:`, JSON.stringify(toolResult).substring(0, 200));
                                await ctx.runMutation(api_1.api.toolCallTracking.updateResult, {
                                    toolCallId,
                                    result: toolResult,
                                    status: toolResult?.success === false
                                        ? "FAILED"
                                        : "COMPLETED",
                                });
                ...
                if (planSteps.length > 0) {
                    ...
                    const progressedPlanThisRound = completedPlanSteps.size > completedPlanBefore;
                    if (!planComplete && !progressedPlanThisRound) {
                        ...
                        if (step) {
                            ...
                            if (!safeForcedTools.has(step.toolName)) {
                                ...
                                const forcedArgs = {
                                    ...step.args,
                                    explanation: step.args.explanation ??
                                        `Forced execution of step ${nextIdx + 1}/${planSteps.length} from user tool plan`,
                                };
                                await ensureToolTracking(forcedToolCallId, step.toolName, forcedArgs);
                                const candidateTaskWorkspace = task?.workspacePath;
                                const workspacePathOverride = candidateTaskWorkspace || "/workspace";
                                const serverUrl = process.env.SHADOW_SERVER_URL || "http://localhost:4000";
                                const toolApiKey = process.env.CONVEX_TOOL_API_KEY || "shadow-internal-tool-key";
                                ...
                                const callToolApiWithWorkspaceFallback = async () => {
                                    const callOnce = async (workspacePath) => {
                                        const resp = await fetch(`${serverUrl}/api/tools/${args.taskId}/${step.toolName}`, {
                                            method: "POST",
                                            headers: {
                                                "Content-Type": "application/json",
                                                "x-convex-tool-key": toolApiKey,
                                ...
                                let forcedResult;
                                try {
                                    const toolDef = aiTools[step.toolName];
                                    // CRITICAL: For scratchpad tasks, ALWAYS use Convex-native tool execution
                                    const isScratchpad = task?.isScratchpad === true;
                                ...
                                await ctx.runMutation(api_1.api.toolCallTracking.updateResult, {
                                    ...
                                    result: forcedResult,
                                    status: forcedResult?.success === false
                                        ? "FAILED"
                                        : "COMPLETED",
                                });
                                await ctx.runMutation(api_1.api.messages.appendStreamDelta, {
                                ...
                                completedToolSignatures.add(makeToolSignature(step.toolName, forcedArgs));
                                completedPlanSteps.add(nextIdx);
                                appendToolTranscript({
                                    toolCallId: forcedToolCallId,
                                    toolName: step.toolName,
                ...
                console.log(`[STREAMING] [LOOP_DEBUG] Round ${round} progress check: ` +
                ...
                if (hadToolCalls && !hadNewToolExecutions) {
                    ...
                    console.warn(`[STREAMING] [ANTI_LOOP] Round ${round}: All ${roundToolCallIds.size} tool calls were duplicates ` +
                ...
                if (consecutiveDuplicateRounds >= MAX_CONSECUTIVE_DUPLICATE_ROUNDS) {
                    console.error(`[STREAMING] [ANTI_LOOP] Stopping: ${MAX_CONSECUTIVE_DUPLICATE_ROUNDS} consecutive rounds of duplicate tool calls. ` +
                ...
                if (!progressed && roundToolCallIds.size === 0 && !hadTextProgress) {
                    console.log(`[STREAMING] [LOOP_EXIT] No progress in round=${round}; stopping (no tools, no text)`);
                    break;
                }
            ...
            const staleCleanup = await ctx.runMutation(api_1.api.toolCallTracking.failStaleRunning, { messageId });
            if (staleCleanup.marked > 0) {
                console.log(`[STREAMING] Marked ${staleCleanup.marked} stale tool calls as FAILED`);
            }
            // Update trace with success (Best Practice BP002, BP018)
            const streamEndedAt = Date.now();
            await ctx.runMutation(api_1.api.observability.updateTrace, {
                traceId,
                status: "COMPLETED",
                promptTokens: finalUsage?.promptTokens,
                completionTokens: finalUsage?.completionTokens,
            ...
            await ctx.runMutation(api_1.api.observability.recordStreamingMetrics, {
                taskId: args.taskId,
                messageId,
                traceId,
            ...
            console.log(`[STREAMING] Streaming completed successfully, text length: ${accumulatedText.length}, traceId: ${traceId}`);
            return {
                success: true,
                messageId,
                text: accumulatedText,
            ...
            else if (userMessage.includes("No output generated") ||
                userMessage.includes("AI_NoOutputGeneratedError")) {
                userMessage =
                    "The model returned no response. This usually happens due to rate limiting on free-tier models. Please try again or switch to a different model.";
            }
 ...
 */
exports.cancelStream = (0, server_1.action)({
    args: {
...
exports.stopTask = (0, server_1.action)({
    ...
    handler: async (ctx, args) => {
        ...
        const messages = await ctx.runQuery(api_1.api.messages.byTask, {
            taskId: args.taskId,
        });
 ...
 */
exports.resumeStream = (0, server_1.action)({
    args: {
        taskId: values_1.v.id("tasks"),
        fromMessageId: values_1.v.id("chatMessages"),
        prompt: values_1.v.string(),
        model: values_1.v.string(),
        systemPrompt: values_1.v.optional(values_1.v.string()),
        llmModel: values_1.v.optional(values_1.v.string()),
    ...
    handler: async (ctx, args) => {
        ...
        const fromMessage = await ctx.runQuery(api_1.api.messages.get, {
        ...
        await ctx.runMutation(api_1.api.messages.removeAfterSequence, {
            taskId: args.taskId,
            sequence: fromMessage.sequence,
        });
        // Now create a new streaming message using the public API
        return ctx.runAction(api_1.api.streaming.streamChat, {
            taskId: args.taskId,
            prompt: args.prompt,
            model: args.model,
