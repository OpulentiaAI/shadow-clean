...
exports.getOrCreateAssistantForPrompt = (0, server_1.mutation)({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query("chatMessages")
            .withIndex("by_task_promptMessageId", (q) => q.eq("taskId", args.taskId).eq("promptMessageId", args.promptMessageId))
            .first();
        if (existing) {
            console.log(`[MESSAGES] Found existing assistant message for prompt ${args.promptMessageId}: ${existing._id}`);
            return {
