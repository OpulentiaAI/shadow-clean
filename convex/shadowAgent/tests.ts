 ...
 */
export const testAbortResume = action({
  args: {
    taskId: v.id("tasks"),
  },
  handler: async (ctx, args) => {
    ...
    const stoppedTask = await ctx.runQuery(api.tasks.get, { taskId: args.taskId });
 ...
 */
export const testToolCalling = action({
  args: {
    taskId: v.id("tasks"),
  },
  handler: async (ctx, args) => {
    ...
    const tools = createAgentTools(ctx as any, args.taskId, (task as any)?.workspacePath);

    // Ask agent to list files (should trigger list_dir tool)
    const result = await thread.generateText(
      { prompt: "Please list the files in the current directory using the list_dir tool." },
      { 
        tools,
        maxSteps: 3,
        maxTokens: 1000,
...
export const runAllTests = action({
  ...
  handler: async (ctx, args) => {
    ...
    const results: Array<{ test: string; result: string; details?: any }> = [];

    try {
      // Test 1: Conversation History
      const historyResult = await ctx.runAction(api.shadowAgent.tests.testConversationHistory, {
        taskId: args.taskId,
      });
      results.push({
        test: "CONVERSATION_HISTORY",
    ...
    try {
      // Test 2: Abort/Resume
      const abortResult = await ctx.runAction(api.shadowAgent.tests.testAbortResume, {
        taskId: args.taskId,
      });
      results.push({
        test: "ABORT_RESUME",
    ...
    try {
      // Test 3: Tool Calling
      const toolResult = await ctx.runAction(api.shadowAgent.tests.testToolCalling, {
        taskId: args.taskId,
      });
      results.push({
        test: "TOOL_CALLING",
