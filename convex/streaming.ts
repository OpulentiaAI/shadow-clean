  ...
  handler: async (ctx, args): Promise<StreamChatWithToolsResult> => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `[STREAMING] streamText promise resolved, processing stream...`
        );
      } catch (streamTextError) {
        console.error(
          `[STREAMING] streamText failed immediately:`,
          streamTextError
        );
      ...
      console.log(
        `[STREAMING] Stream iteration complete, processed ${partCount} parts`
      );
      console.log(
